...
if doAuto then
  ...
  if confirm then
    ...
    sleepWithStatus(300,"Found/Clicked Confirm button to Clear screen", nil, 0.7);
  end

  if fateCard then -- Found Generic/Brown card, so change it
    srClickMouse(fateCard[0], fateCard[1])
    if moreFeedback then
      sleepWithStatus(300,"Found/Clicked Generic/Brown Fate Card", nil, 0.7);
    end
  ...
  elseif fateCard2 or (fateCard3 and doSeven) or fateCard4 then -- Found an existing desirable card; keep it!
    if moreFeedback then
      sleepWithStatus(300,"Desirable card ALREADY LOADED", nil, 0.7);
    end
  ...
  elseif fateDetails then
    if moreFeedback then
      sleepWithStatus(300,"Undesirable card seems loaded, CHANGE IT!", nil, 0.7);
    end
...
function changeFateCards()
    while 1 do
      sleepWithStatus(300,"Searching for desirable cards", nil, 0.7);
