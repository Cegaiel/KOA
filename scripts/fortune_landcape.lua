...
moreFeedback = false;
abortCounter = 0;
maxAbortCounter = 10; -- How many times to wait when "nothing to do" is encountered
function doit()
...
if doAuto then
  ...
  while not lsShiftHeld() do
    sleepWithStatus(100,"Tap Shift to Change Fate Cards."..elapsed(), nil, 0.7);
  end

  while lsShiftHeld() do
    sleepWithStatus(100,"Release Shift Key"..elapsed(), nil, 0.7);
  end
end
  ...
  local notokens = srFindImage("fortune/insufficient_tokens.png", Tolerance);

  if confirm then
    srClickMouse(confirm[0], confirm[1])
    sleepWithStatus(300,"Found/Clicked Confirm button to Clear screen"..elapsed(), nil, 0.7);
  end

  if fateCard then -- Found Generic/Brown card, so change it
    srClickMouse(fateCard[0], fateCard[1])
    if moreFeedback then
      sleepWithStatus(300,"Found/Clicked Generic/Brown Fate Card"..elapsed(), nil, 0.7);
    end

    changeFateCards();

    if rollDice() then
      sleepWithStatus(diceDelay,"Rolling Dice; Waiting on Movement ..."..elapsed(), nil, 0.7);
      -- Check if rewards window is open and close it
      checkRewardsConfirmButton();
    else
      sleepWithStatus(diceDelay/2,"Dice not Found!"..elapsed(), nil, 0.7);
    end


  elseif fateCard2 or (fateCard3 and doSeven) or fateCard4 then -- Found an existing desirable card; keep it!
    if moreFeedback then
      sleepWithStatus(300,"Desirable card ALREADY LOADED"..elapsed(), nil, 0.7);
    end
  ...
  elseif fateDetails then
    if moreFeedback then
      sleepWithStatus(300,"Undesirable card seems loaded, CHANGE IT!"..elapsed(), nil, 0.7);
    end
...
function changeFateCards()
    while 1 do
      sleepWithStatus(300,"Searching for desirable cards"..elapsed(), nil, 0.7);
	  ...
	  if majorHaul then
	    cardName = "MAJOR HAUL"
	  elseif onHouse then
	    cardName = "ON THE HOUSE"
	  elseif doSeven then
	    cardName = "LUCKY SEVEN"
	  else
	    cardName = "UNKNOWN???"
	  end
          sleepWithStatus(500,"Found Desirable Card; KEEP IT !\n\nCard Name: " .. cardName ..elapsed(), nil, 0.7);
          ESCKey();
          break;
        end
      end -- if changeCard
...
function ESCKey()
  ...
  srKeyUp(VK_ESCAPE);
  --lsSleep(100);
  sleepWithStatus(100,"Escape Key (Close Windows)"..elapsed(), nil, 0.7);
end
...
function checkRewardsConfirmButton()
  ...
  if notokens then
    abortCounter = maxAbortCounter
    sleepWithStatus(5000,"No Tokens Left! Aborting ..." ..elapsed(), nil, 0.7);
